---
title: "Algebra and Statistical Methods for Big Data"
author: 
- name: Dolors Pelegri
  affiliation: 
  - &isglobal Barcelona Institute for Global Health (ISGlobal)
...
output:
  ...
  BiocStyle::pdf_document:
    toc_float: true
abstract: |
  Description of functions to perform matrix operations, algebra and some statistical methods using HDF5 data files and _DelayedMatrix_ objects.
vignette: |
...
The package requires some R packages that can be installed by

```{r eval=FALSE}
install.packages("Matrix", "RcppEigen", "RSpectra", "DT")
```

and some [Bioconductor](https://bioconductor.org/) ones:

```{r eval=FALSE}
BiocManager::install(c("beachmat", "HDF5Array", "DelayedArray", "rhdf5"))
```
...
This package implements several matrix operations using HDF5 data files and `DelayedMatrix` objects. `BDSM` also contains functions to perform some basic algebra operations that can be used to carry out some statistical analyses using standard methodologies such as principal component analyses or least squares estimation. The package also contains specific statistical method mainly used in `omic` data analysis such as lasso regression. 
...
This package is also required to reproduce this vignette
